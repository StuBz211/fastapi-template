...
from auth.dependencies import AuthDependency, validate_permissions
...
async def user_detail_me(
    auth: AuthDependency = Depends(), session: AsyncSession = Depends(get_session)
) -> schemas.User:
...
async def user_update(
    user_data: schemas.UserUpdate, auth: AuthDependency = Depends(), session: AsyncSession = Depends(get_session)
) -> schemas.User:
    user_id = auth.get_user_pk()
...
async def user_activate(
    token: str, auth: AuthDependency.no_validate = Depends(), session: AsyncSession = Depends(get_session)
) -> schemas.User:
